...
#include "SGEditorSubsystem.generated.h"

class FTokenizedMessage;
class ISGAttributeReferencerHandler;
class IMessageToken;
class IToolkit;
class UEdGraphPin;
 ...
 */
UCLASS()
class SAGASTATSEDITOR_API USGEditorSubsystem : public UEditorSubsystem
{
	GENERATED_BODY()
...
public:
	...
	TMap<FName, TSharedPtr<ISGAttributeReferencerHandler>> RegisteredHandlers;
	...
	virtual void RegisterReferencerHandler(const FName& InClassName, const TSharedPtr<ISGAttributeReferencerHandler>& InHandler);
	...
	TSharedPtr<ISGAttributeReferencerHandler> FindAssetDependencyHandler(const FName& InReferencer, TWeakObjectPtr<UObject>& OutDefaultObject);
