...
#include "ReferencerHandlers/SGGameplayEffectReferencerHandler.h"
...
#define LOCTEXT_NAMESPACE "SGEditorSubsystem"

void USGEditorSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	FSGDelegates::OnVariableRenamed.AddUObject(this, &USGEditorSubsystem::HandleAttributeRename);
	FSGDelegates::OnPreCompile.AddUObject(this, &USGEditorSubsystem::HandlePreCompile);
	...
	FSGDelegates::OnPostCompile.AddUObject(this, &USGEditorSubsystem::HandlePostCompile);

	// Register handlers
	RegisterReferencerHandler(TEXT("GameplayEffect"), FSGGameplayEffectReferencerHandler::Create());
}

void USGEditorSubsystem::Deinitialize()
{
	Super::Deinitialize();
...
USGEditorSubsystem& USGEditorSubsystem::Get()
{
	check(GEditor);
	return *GEditor->GetEditorSubsystem<USGEditorSubsystem>();
}

void USGEditorSubsystem::RegisterReferencerHandler(const FName& InClassName, const TSharedPtr<ISGAttributeReferencerHandler>& InHandler)
{
	RegisteredHandlers.Emplace(InClassName, InHandler);
}

void USGEditorSubsystem::UnregisterReferencerHandler(const FName& InClassName)
{
...
TSharedPtr<ISGAttributeReferencerHandler> USGEditorSubsystem::FindAssetDependencyHandler(const FName& InPackageName, TWeakObjectPtr<UObject>& OutDefaultObject)
{
...
void USGEditorSubsystem::HandleAttributeRename(const FName& InPackageName, const FName& InOldPropertyName, const FName& InNewPropertyName)
{
	if (bIsHandlingRename)
	{
		SG_EDITOR_LOG(Verbose, TEXT("USGEditorSubsystem::HandleAttributeRename - Handling rename already. PackageName: %s, OldName: %s, NewName: %s"), *InPackageName.ToString(), *InOldPropertyName.ToString(), *InNewPropertyName.ToString())
		return;
	}
...
void USGEditorSubsystem::HandlePreCompile(const FName& InPackageName)
{
	SG_EDITOR_LOG(Verbose, TEXT("USGEditorSubsystem::HandlePreCompile InPackageName: %s"), *InPackageName.ToString())
	...
	for (const TPair<FName, TSharedPtr<ISGAttributeReferencerHandler>>& RegisteredHandler : RegisteredHandlers)
	{
		TSharedPtr<ISGAttributeReferencerHandler> Handler = RegisteredHandler.Value;
		if (Handler.IsValid())
		{
	...
	{
		FSGAttributeReferencerPayload Payload;
		Payload.PackageName = InPackageName.ToString();
		TSharedPtr<ISGAttributeReferencerHandler> ReferencerHandler = FindAssetDependencyHandler(Referencer.PackageName, Payload.DefaultObject);
	...
	for (TObjectIterator<UK2Node> NodeIt; NodeIt; ++NodeIt)
	{
		...
		{
			TSharedPtr<ISGAttributeReferencerHandler> Handler = RegisteredHandlers.FindChecked(NodeClass->GetFName());
...
void USGEditorSubsystem::GetReferencers(const FName& InPackageName, TArray<FAssetDependency>& OutReferencers, TArray<FAssetData>& OutAssetsData)
{
	const TArray<UClass*> AllowedClasses;
	GetReferencers(InPackageName, OutReferencers, OutAssetsData, AllowedClasses);
}

void USGEditorSubsystem::GetReferencers(const FName& InPackageName, TArray<FAssetDependency>& OutReferencers, TArray<FAssetData>& OutAssetsData, const TArray<UClass*>& InAllowedClasses)
{
	...
	SG_EDITOR_LOG(Verbose, TEXT("USGEditorSubsystem::GetReferencers Referencers: %d"), Referencers.Num())
	...
	for (const FAssetDependency& Referencer : Referencers)
	{
		...
		SG_EDITOR_LOG(Verbose, TEXT("\t Referencer: %s"), *ReferencerName.ToString())
...
TArray<FAssetDependency> USGEditorSubsystem::GetReferencersOfClass(const TArray<FAssetDependency>& InReferencers, const UClass* InFilterClass)
{
...
void USGEditorSubsystem::GetGameplayEffectReferencers(const FName& InPackageName, TArray<FAssetDependency>& OutReferencers, TArray<FAssetData>& OutAssetsData)
{
	GetReferencers(InPackageName, OutReferencers, OutAssetsData, { UGameplayEffect::StaticClass() });
}

UGameplayEffect* USGEditorSubsystem::GetGameplayEffectCDO(const FString& InPackageName)
{
	SG_EDITOR_LOG(Verbose, TEXT("USGEditorSubsystem::GetGameplayEffectCDO - Try load with %s"), *InPackageName)

	// Try load class and access to CDO
	const UClass* LoadClass = StaticLoadClass(UObject::StaticClass(), nullptr, *InPackageName);
	SG_EDITOR_LOG(Verbose, TEXT("\t LoadClass: %s"), *GetNameSafe(LoadClass))
	if (!LoadClass)
	{
		return nullptr;
	}
	...
	SG_EDITOR_LOG(Verbose, TEXT("\t LoadClass CDO: %s (Class: %s)"), *GetNameSafe(CDO), *GetNameSafe(CDO->GetClass()))
...
UGameplayEffect* USGEditorSubsystem::GetGameplayEffectCDO(const FAssetDependency& InAssetDependency)
{
	...
	SG_EDITOR_LOG(VeryVerbose, TEXT("\t Try load with %s"), *PackageName)
	return GetGameplayEffectCDO(PackageName);
}

FString USGEditorSubsystem::GetLoadClassPackagePath(const FString& InPackageName)
{
	FString Result;
	...
	{
		SG_EDITOR_LOG(Warning, TEXT("USGEditorSubsystem::GetGameplayEffectCDO Unable to split %s by /"), *InPackageName)
		return Result;
	}

	if (Right.IsEmpty())
	{
		SG_EDITOR_LOG(Warning, TEXT("USGEditorSubsystem::GetGameplayEffectCDO Empty right"), *InPackageName)
		return Result;
	}
...
void USGEditorSubsystem::UpdateK2NodeReferencers(const FName& InPackageName, const FName& InOldPropertyName, const FName& InNewPropertyName)
{
	SG_EDITOR_LOG(Verbose, TEXT("USGEditorSubsystem::UpdateK2NodeReferencers InPackageName: %s, InOldPropertyName: %s, InNewPropertyName: %s"), *InPackageName.ToString(), *InOldPropertyName.ToString(), *InNewPropertyName.ToString())
	
	UObject* OwnerAttributeSet = LoadObject<UObject>(nullptr, *InPackageName.ToString());
	if (!OwnerAttributeSet)
	{
		SG_EDITOR_LOG(Warning, TEXT("USGEditorSubsystem::UpdateK2NodeReferencers - Cannot load AttributeSet class from %s"), *InPackageName.ToString())
		return;
	}
	...
	{
		SG_EDITOR_LOG(Warning, TEXT("USGEditorSubsystem::UpdateK2NodeReferencers - Cannot load AttributeSet class Blueprint from %s"), *InPackageName.ToString())
		return;
	}
...
TArray<USGEditorSubsystem::FPinToModify> USGEditorSubsystem::GetPinsToModify(const FString& InPackageName, const FString& InOldPropertyName, const FString& InNewPropertyName)
{
		...
		for (UEdGraphPin* Pin : Node->Pins)
		{
			...
			if (Pin->Direction == EGPD_Input)
			{
				...
				if (!ParseAttributeFromDefaultValue(DefaultValue, PackageName, AttributeName))
				{
					SG_EDITOR_LOG(
						VeryVerbose,
						TEXT("USGEditorSubsystem::UpdateK2NodeReferencers - Cannot parse attribute from DefaultValue: %s (Pin: %s, Node: %s, Graph: %s)"),
						*DefaultValue,
						*Pin->GetName(),
						*GetNameSafe(Node),
...
void USGEditorSubsystem::HandlePins(const TArray<FPinToModify>& InPinsToModify, const UBlueprint* InOwnerAttributeSetBP, const FName& InNewPropertyName)
{
	SG_EDITOR_LOG(Verbose, TEXT("USGEditorSubsystem::UpdateK2NodeReferencers - Test InPinsToModify: %d"), InPinsToModify.Num())
	...
	for (const FPinToModify& PinToModify : InPinsToModify)
	{
		...
		{
			SG_EDITOR_LOG(Warning, TEXT("USGEditorSubsystem::UpdateK2NodeReferencers - NO GENERATED CLASS (InOwnerAttributeSetBP: %s"), *GetNameSafe(InOwnerAttributeSetBP))
			continue;
		}
		...
		SG_EDITOR_LOG(Verbose, TEXT("\t USGEditorSubsystem::UpdateK2NodeReferencers - PinToModify Blueprint: %s"), *GetNameSafe(Blueprint))
		SG_EDITOR_LOG(Verbose, TEXT("\t USGEditorSubsystem::UpdateK2NodeReferencers - PinToModify Pin: %s"), *Pin->GetName())

		if (FProperty* Prop = FindFProperty<FProperty>(InOwnerAttributeSetBP->GeneratedClass, InNewPropertyName))
		{
			SG_EDITOR_LOG(Verbose, TEXT("\t Prop: %s (Owner: %s)"), *GetNameSafe(Prop), *InOwnerAttributeSetBP->GeneratedClass->GetName())
			...
			if (FinalValue != DefaultValue && Pin->GetSchema())
			{
				...
				SG_EDITOR_LOG(Verbose, TEXT("\t %s"), *Message->ToText().ToString())
	...
	SG_EDITOR_LOG(Verbose, TEXT("USGEditorSubsystem::UpdateK2NodeReferencers - BlueprintsModified: %d"), BlueprintsModified.Num())
	for (UBlueprint* Blueprint : BlueprintsModified)
	{
		...
		SG_EDITOR_LOG(Verbose, TEXT("\t MarkBlueprintAsStructurallyModified Blueprint: %s"), *Blueprint->GetName())
		FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(Blueprint);

		FString PathName = Blueprint->GetPathName();
		SG_EDITOR_LOG(Verbose, TEXT("\t PathName: %s"), *PathName)
		...
		{
			SG_EDITOR_LOG(Verbose, TEXT("\t Blueprint: %s is the same. Needs compile again."), *Blueprint->GetName())
			...
			{
				SG_EDITOR_LOG(Display, TEXT("Blueprint compiled successfully (%s)"), *Blueprint->GetName());
			}
			else if (Blueprint->Status == BS_UpToDateWithWarnings)
			{
				SG_EDITOR_LOG(Warning, TEXT("Blueprint compiled successfully with warnings(%s)"), *Blueprint->GetName());
			}
			else if (Blueprint->Status == BS_Error)
			{
				SG_EDITOR_LOG(Error, TEXT("Blueprint failed to compile (%s)"), *Blueprint->GetName());
			}
			else
			{
				SG_EDITOR_LOG(Error, TEXT("Blueprint is in an unexpected state after compiling (%s)"), *Blueprint->GetName());
			}
		}
	}
}

void USGEditorSubsystem::HandleMessageLogLinkActivated(const TSharedRef<IMessageToken>& InToken)
{
...
bool USGEditorSubsystem::ParseAttributeFromDefaultValue(const FString& InDefaultValue, FString& OutPackageName, FString& OutAttributeName)
{
	SG_EDITOR_LOG(Verbose, TEXT("USGEditorSubsystem::ParsePackageNameFromDefaultValue InDefaultValue: %s"), *InDefaultValue)
...
//Feature Begin Attribute In subclass of AttributeSet
bool USGEditorSubsystem::ParseAttributeOwnerClassNameFromDefaultValue(const FString& InDefaultValue, FString& OutAttributeOwnerClassName)
{
	SG_EDITOR_LOG(Verbose, TEXT("USGEditorSubsystem::ParseAttributeOwnerClassNameFromDefaultValue InDefaultValue: %s"), *InDefaultValue)
	FString DefaultValue = InDefaultValue;
...
void USGEditorSubsystem::UpdateReferencers(TArray<FAssetData> InReferencers, const FName& InPackageName, const FName& InOldPropertyName, const FName& InNewPropertyName)
{
	SG_EDITOR_LOG(Verbose, TEXT("USGEditorSubsystem::UpdateReferencers Referencer Assets: %d"), InReferencers.Num())
	...
	for (const FAssetData& Referencer : InReferencers)
	{
		...
		TSharedPtr<ISGAttributeReferencerHandler> Handler = FindAssetDependencyHandler(Referencer.PackageName, Payload.DefaultObject);
...
void USGEditorSubsystem::HandlePostCompile(const FName& InPackageName)
{
	SG_EDITOR_LOG(Verbose, TEXT("USGEditorSubsystem::HandlePostCompile InPackageName: %s"), *InPackageName.ToString())
	...
	if (GEditor)
	{
		GEditor->GetTimerManager()->SetTimerForNextTick(FTimerDelegate::CreateStatic(&USGEditorSubsystem::HandleNextTick, InPackageName, ClosedPackageNames));
	}
	
	OpenClosedEditors(ClosedPackageNames);
}

void USGEditorSubsystem::CloseEditors(const TArray<FAssetData>& InAssets, TArray<FAssetData>& OutClosedAssets)
{
	for (const FAssetData& AssetData : InAssets)
	{
		const UObject* Asset = AssetData.FastGetAsset();
		SG_EDITOR_LOG(Verbose, TEXT("\tClosing asset: %s (UObject: %s)"), *AssetData.GetFullName(), *GetNameSafe(Asset));
...
void USGEditorSubsystem::OpenClosedEditors(const TArray<FString>& InClosedAssets)
{
	UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>();
	if (!AssetEditorSubsystem)
	{
		SG_EDITOR_LOG(Error, TEXT("USGEditorSubsystem::OpenClosedEditors failed because AssetEditorSubsystem is null"));
		return;
	}
...
bool USGEditorSubsystem::IsAssetCurrentlyBeingEdited(const TSharedPtr<IToolkit>& InAssetEditor, const UObject* InAsset)
{
	if (!InAsset)
	{
		return false;
...
void USGEditorSubsystem::HandleNextTick(FName InPackageName, const TArray<FString> InClosedAssets)
{
