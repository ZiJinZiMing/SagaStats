...
{
	check(InBlueprint && InBlueprint->IsA(USSAttributeSetBlueprint::StaticClass()));
	USSBlueprintFactory* Factory = NewObject<USSBlueprintFactory>();
	Factory->ParentClass = TSubclassOf<USSAttributeSetBlueprintBase>(*InBlueprint->GeneratedClass);
	return Factory;
}
