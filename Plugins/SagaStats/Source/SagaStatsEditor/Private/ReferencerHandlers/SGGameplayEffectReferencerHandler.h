 ...
 */
class FSGGameplayEffectReferencerHandler : public ISGAttributeReferencerHandler
{
public:
	...
	virtual ~FSGGameplayEffectReferencerHandler() override;
...
protected:
	 ...
	 */
	virtual bool UpdateModifiers(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FSGAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages);
	/** Common code shared between modifiers backing attribute and duration */
	static bool UpdateGameplayEffectModifierMagnitude(FGameplayModifierInfo& InOutModifier, const FAttributeReference& CachedAttribute, const UBlueprint* InBlueprint, const FSGAttributeReferencerPayload& InPayload);

	static bool UpdateGameplayEffectModifierMagnitude(FGameplayEffectModifierMagnitude& InOutModifierMagnitude, const FAttributeReference& CachedAttribute, const UBlueprint* InBlueprint, const FSGAttributeReferencerPayload& InPayload);
	virtual bool UpdateModifiersBackingAttribute(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FSGAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages);
	virtual bool UpdateDuration(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FSGAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages);
	virtual bool UpdateCues(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FSGAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages);
	virtual bool UpdateRemoveGameplayEffectQuery(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FSGAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages);
	virtual bool UpdateImmunityEffectQuery(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FSGAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages);
	...
	virtual bool UpdateEffectQuery(
		...
		const FSGAttributeReferencerPayload& InPayload,
