...
#define LOCTEXT_NAMESPACE "USSAttributeSetBlueprintBase"

TMap<FString, FString> USSAttributeSetBlueprintBase::RepNotifierHandlerNames = {
	{ TEXT("FGameplayAttributeData"), TEXT("HandleRepNotifyForGameplayAttributeData") },
	{ TEXT("FSSGameplayClampedAttributeData"), TEXT("HandleRepNotifyForGameplayClampedAttributeData") }
};
...
float FSSClampDefinition::GetValueForClamping(const UAttributeSet* InOwnerSet) const
{
	check(InOwnerSet);
...
bool FSSClampDefinition::GetValueForClamping(const UAttributeSet* InOwnerSet, float& OutValue) const
{
	check(InOwnerSet);

	if (ClampType == ESSClampingType::None)
	{
		OutValue = 0.f;
		return false;
	}
	
	if (ClampType == ESSClampingType::Float)
	{
		OutValue = Value;
		return true;
	}

	if (ClampType == ESSClampingType::AttributeBased && Attribute.IsValid())
	{
...
USSAttributeSetBlueprintBase::USSAttributeSetBlueprintBase(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{

}

void USSAttributeSetBlueprintBase::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
...
void USSAttributeSetBlueprintBase::InitFromMetaDataTable(const UDataTable* DataTable)
{
...
bool USSAttributeSetBlueprintBase::PreGameplayEffectExecute(FGameplayEffectModCallbackData& Data)
{
...
bool USSAttributeSetBlueprintBase::K2_PreGameplayEffectExecute_Implementation(const FGameplayAttribute& InAttribute, const FSSAttributeSetExecutionData& InData)
{
	return true;
}

void USSAttributeSetBlueprintBase::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (!bSuccessfullyFoundAttribute)
	{
		SS_LOG(
			Warning,
			TEXT("USSAttributeSetBlueprintBase::PostGameplayEffectExecute - Error reading %s value (clamping if any was expected couldn't be performed)"),
...
void USSAttributeSetBlueprintBase::PreAttributeChange(const FGameplayAttribute& Attribute, float& OutValue)
{
...
void USSAttributeSetBlueprintBase::PostAttributeChange(const FGameplayAttribute& Attribute, const float OldValue, const float NewValue)
{
	Super::PostAttributeChange(Attribute, OldValue, NewValue);
	K2_PostAttributeChange(Attribute, OldValue, NewValue);
}

void USSAttributeSetBlueprintBase::PreAttributeBaseChange(const FGameplayAttribute& Attribute, float& OutValue) const
{
...
void USSAttributeSetBlueprintBase::PostAttributeBaseChange(const FGameplayAttribute& Attribute, const float OldValue, const float NewValue) const
{
	Super::PostAttributeBaseChange(Attribute, OldValue, NewValue);
	K2_PostAttributeBaseChange(Attribute, OldValue, NewValue);
}

void USSAttributeSetBlueprintBase::OnAttributeAggregatorCreated(const FGameplayAttribute& Attribute, FAggregator* NewAggregator) const
{
...
void USSAttributeSetBlueprintBase::PrintDebug()
{
	Super::PrintDebug();
}
...
void USSAttributeSetBlueprintBase::ClampAttributeValue(const FGameplayAttribute Attribute, const float MinValue, const float MaxValue)
{
	bool bSuccessfullyFoundAttribute = true;
	...
	if (!bSuccessfullyFoundAttribute)
	{
		SS_LOG(
			Warning,
			TEXT("USSAttributeSetBlueprintBase::ClampAttributeValue - Error reading %s value (clamping couldn't be performed)"),
			*Attribute.GetName()
		)
		return;
	}
...
float USSAttributeSetBlueprintBase::K2_GetAttributeValue(FGameplayAttribute Attribute, bool& bSuccessfullyFoundAttribute) const
{
	return GetAttributeValue(Attribute, bSuccessfullyFoundAttribute);
}

float USSAttributeSetBlueprintBase::GetAttributeValue(const FGameplayAttribute& Attribute, bool& bSuccessfullyFoundAttribute) const
{
	const UAbilitySystemComponent* ASC = GetOwningAbilitySystemComponent();
	if (!ASC)
	{
		SS_LOG(Warning, TEXT("USSAttributeSetBlueprintBase::GetAttributeValue - Unable to get attribute value for %s because ASC is invalid"), *Attribute.GetName())
...
float USSAttributeSetBlueprintBase::K2_GetAttributeBaseValue(FGameplayAttribute Attribute, bool& bSuccessfullyFoundAttribute) const
{
	return GetAttributeBaseValue(Attribute, bSuccessfullyFoundAttribute);
}

float USSAttributeSetBlueprintBase::GetAttributeBaseValue(const FGameplayAttribute& Attribute, bool& bSuccessfullyFoundAttribute) const
{
	const UAbilitySystemComponent* ASC = GetOwningAbilitySystemComponent();
	if (!ASC)
	{
		SS_LOG(Warning, TEXT("USSAttributeSetBlueprintBase::GetAttributeBaseValue - Unable to get attribute value for %s because ASC is invalid"), *Attribute.GetName())
		bSuccessfullyFoundAttribute = false;
		return 0.f;
	}
...
void USSAttributeSetBlueprintBase::K2_SetAttributeValue(const FGameplayAttribute Attribute, const float NewValue)
{
	SetAttributeValue(Attribute, NewValue);
}

void USSAttributeSetBlueprintBase::SetAttributeValue(const FGameplayAttribute& Attribute, const float NewValue) const
{
	UAbilitySystemComponent* ASC = GetOwningAbilitySystemComponent();
	if (!ASC)
	{
		SS_LOG(Warning, TEXT("USSAttributeSetBlueprintBase::SetAttributeValue - Unable to set value for %s because ASC is invalid"), *Attribute.GetName())
		return;
	}
...
bool USSAttributeSetBlueprintBase::IsGameplayAttributeDataClampedProperty(const FProperty* Property)
{
...
AActor* USSAttributeSetBlueprintBase::K2_GetOwningActor() const
{
	return GetOwningActor();
}

UAbilitySystemComponent* USSAttributeSetBlueprintBase::K2_GetOwningAbilitySystemComponent() const
{
	return GetOwningAbilitySystemComponent();
}

FGameplayAbilityActorInfo USSAttributeSetBlueprintBase::K2_GetActorInfo() const
{
	FGameplayAbilityActorInfo* ActorInfo = GetActorInfo();
	if (!ActorInfo)
	{
		SS_LOG(Error, TEXT("USSAttributeSetBlueprintBase::K2_GetActorInfo - Unable to get actor info pointer"))
...
void USSAttributeSetBlueprintBase::HandleRepNotifyForGameplayAttribute(const FName InPropertyName)
{
	...
	if (!ThisProperty)
	{
		SS_LOG(
			Warning,
			TEXT("USSAttributeSetBlueprintBase::HandleRepNotifyForGameplayAttribute - Invalid InPropertyName (%s) - could not find in %s"),
	...
	const FGameplayAttributeData* AttributeData = Attribute.GetGameplayAttributeData(this);
	{
		...
		{
			SS_LOG(Error, TEXT("USSAttributeSetBlueprintBase::HandleRepNotifyForGameplayAttribute - %s"), *ErrorMessage)
			return;
		}
	}
...
void USSAttributeSetBlueprintBase::HandleRepNotifyForGameplayAttributeData(const FGameplayAttributeData& InAttribute)
{
...
void USSAttributeSetBlueprintBase::HandleRepNotifyForGameplayClampedAttributeData(const FSSGameplayClampedAttributeData& InAttribute)
{
	HandleRepNotifyForGameplayAttributeData(InAttribute);
}

void USSAttributeSetBlueprintBase::BeginDestroy()
{
...
void USSAttributeSetBlueprintBase::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
...
void USSAttributeSetBlueprintBase::PreNetReceive()
{
	Super::PreNetReceive();
	...
	SS_LOG(VeryVerbose, TEXT("USSAttributeSetBlueprintBase::PreNetReceive ... ReplicatedProps: %d"), ReplicatedProps.Num())
...
EDataValidationResult USSAttributeSetBlueprintBase::IsDataValid(FDataValidationContext& Context) const
{
...
EDataValidationResult USSAttributeSetBlueprintBase::IsDataValid(TArray<FText>& ValidationErrors)
{
...
EDataValidationResult USSAttributeSetBlueprintBase::IsDataValidBlueprintEditor(TArray<FText>& ValidationErrors) const
{
	...
	SS_LOG(Verbose, TEXT("USSAttributeSetBlueprintBase::IsDataValid TestObject: %s"), *GetNameSafe(TestObject))
	SS_LOG(Verbose, TEXT("USSAttributeSetBlueprintBase::IsDataValid EditorName: %s"), *EditorName)
...
EDataValidationResult USSAttributeSetBlueprintBase::IsDataValidRepNotifies(TArray<FText>& ValidationErrors) const
{
	EDataValidationResult Result = EDataValidationResult::NotValidated;
	...
	SS_LOG(VeryVerbose, TEXT("USSAttributeSetBlueprintBase::IsDataValidRepNotifies - Check for rep notifies validity ..."))
	
	const UBlueprint* Blueprint = UBlueprint::GetBlueprintFromClass(GetClass());
	SS_LOG(VeryVerbose, TEXT("USSAttributeSetBlueprintBase::IsDataValidRepNotifies - Blueprint: %s"), *GetNameSafe(Blueprint))
	if (!Blueprint)
	{
		return Result;
	}
	...
	SS_LOG(VeryVerbose, TEXT("USSAttributeSetBlueprintBase::IsDataValidRepNotifies - ReplicatedProperties: %d"), ReplicatedProperties.Num())
	...
	for (const FProperty* Property : RepNotifiedProperties)
	{
		...
		if (const UEdGraph* NotifyGraph = *FoundNotifyGraph)
		{
			...
			SS_LOG(VeryVerbose, TEXT("USSAttributeSetBlueprintBase::IsDataValidRepNotifies - FunctionNodes: %d (Graph: %s, Property: %s)"), FunctionNodes.Num(), *NotifyGraph->GetName(), *Property->GetName())
...
bool USSAttributeSetBlueprintBase::IsNodeWiredToEntry(const UK2Node* InNode)
{
	if (!InNode)
	{
		return false;
	...
	for (const UEdGraphPin* LinkedTo : InputPin->LinkedTo)
	{
		...
		SS_LOG(VeryVerbose, TEXT("USSAttributeSetBlueprintBase::IsNodeWiredToEntry - LinkedTo: %s, OwningNode: %s, InputPin: %s"), *LinkedTo->GetName(), *GetNameSafe(OwningNode), *InputPin->GetName())
	}

	return bHasPinWiredToEntry;
}

UEdGraphPin* USSAttributeSetBlueprintBase::FindGraphNodePin(const UEdGraphNode* InNode, const EEdGraphPinDirection InDirection)
{
...
TMap<FString, TSharedPtr<FAttributeMetaData>> USSAttributeSetBlueprintBase::GetAttributesMetaData() const
{
	return AttributesMetaData;
}

void USSAttributeSetBlueprintBase::InitClampedAttributeDataProperties()
{
...
void USSAttributeSetBlueprintBase::InitDataTableProperties(const UDataTable* DataTable)
{
	if (!DataTable)
	{
	...
	static const FString Context = FString(TEXT("USSAttributeSetBlueprintBase::BindToMetaDataTable"));
...
float USSAttributeSetBlueprintBase::GetClampedValueForClampedProperty(const FGameplayAttribute& Attribute, const float InValue) const
{
	...
	if (IsGameplayAttributeDataClampedProperty(Attribute.GetUProperty()))
	{
		if (const FSSGameplayClampedAttributeData* Clamped = static_cast<const FSSGameplayClampedAttributeData*>(Attribute.GetGameplayAttributeData(this)))
		{
			...
			{
				// SS_LOG(VeryVerbose, TEXT("USSAttributeSetBlueprintBase::GetClampedValueForClampedProperty - Clamp %f between %f and %f"), NewValue, MinValue, MaxValue);
				NewValue = FMath::Clamp(NewValue, MinValue, MaxValue);
			}
			// If only min is valid, then clamp using the lower bound only
			else if (bIsMinValid)
			{
				// SS_LOG(VeryVerbose, TEXT("USSAttributeSetBlueprintBase::GetClampedValueForClampedProperty - Clamp %f using only min %f (max invalid)"), NewValue, MinValue);
				NewValue = FMath::Max(NewValue, MinValue);
			}
			// If only max is valid, then clamp using the higher bound only
			else if (bIsMaxValid)
			{
				// SS_LOG(VeryVerbose, TEXT("USSAttributeSetBlueprintBase::GetClampedValueForClampedProperty - Clamp %f using only max %f (min invalid)"), NewValue, MaxValue);
			...
			else
			{
				SS_LOG(
					Warning,
					TEXT("USSAttributeSetBlueprintBase::GetClampedValueForClampedProperty - "
...
bool USSAttributeSetBlueprintBase::IsValidAttributeMetadata(const FAttributeMetaData& InAttributeMetadata)
{
	...
	if (AttributesMetaData.Contains(AttributeName))
	{
		...
		if (MetaData.IsValid())
		{
			...
			else
			{
				...
				SS_LOG(
					Verbose,
					TEXT("USSAttributeSetBlueprintBase::GetClampedValueForMetaData - "
...
void USSAttributeSetBlueprintBase::GetAllBlueprintReplicatedProps(TArray<FProperty*>& OutProperties, const EPropertyFlags InCheckFlag) const
{
	GetAllBlueprintReplicatedProps(GetClass(), OutProperties, InCheckFlag);
}

void USSAttributeSetBlueprintBase::GetAllBlueprintReplicatedProps(UClass* InClass, TArray<FProperty*>& OutProperties, const EPropertyFlags InCheckFlag) const
{
...
bool USSAttributeSetBlueprintBase::GetAttributeDataPropertyName(const FGameplayAttributeData& InAttributeData, FString& OutPropertyName)
{
	const UBlueprintGeneratedClass* BPClass = Cast<UBlueprintGeneratedClass>(GetClass());
	if (!BPClass)
	{
