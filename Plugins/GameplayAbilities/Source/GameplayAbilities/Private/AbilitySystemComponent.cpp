		...
		for (UAttributeSet* Set : GetSpawnedAttributes())
		{
			...
			for (const FGameplayAttribute& Attribute : Attributes)
			{
				...
				if (Attribute.IsValid())
				{
					...
					DebugLine(Info, FString::Printf(TEXT("%s.%s %.2f "),*FGameplayAttribute::GetAttributeSetClassName(Set->GetClass()), *Attribute.GetName(), GetNumericAttribute(Attribute)), 4.f, 0.f);
					//ZhangJinming EndChange
				}
			}
		}
