...
//Han Jiping - Begin Change - Cross-module access
/*static*/GAMEPLAYABILITIES_API TAutoConsoleVariable<float> CVarReplayMontageErrorThreshold(TEXT("replay.MontageErrorThreshold"), 0.5f, TEXT("Tolerance level for when montage playback position correction occurs in replays"));
...
/*static*/GAMEPLAYABILITIES_API TAutoConsoleVariable<bool> CVarReplicateMontageNextSectionId(TEXT("AbilitySystem.ReplicateMontageNextSectionId"), true, TEXT("Apply the replicated next section Id to montages when skipping position replication"));
...
bool UAbilitySystemComponent::InternalTryActivateAbility(FGameplayAbilitySpecHandle Handle, FPredictionKey InPredictionKey, UGameplayAbility** OutInstancedAbility, FOnGameplayAbilityEnded::FDelegate* OnGameplayAbilityEndedDelegate, const FGameplayEventData* TriggerEventData)
{
	...
	//Han Jiping - Begin Change - Failure targetdata used by InternalTryActivateAbility
	InternalTryActivateAbilityFailureTargetData.Clear();
	//Han Jiping - End Change
	...
	if (TriggerEventData)
	{
		...
		if (!AbilitySource->ShouldAbilityRespondToEvent(ActorInfo, TriggerEventData, &InternalTryActivateAbilityFailureTargetData))
		//Han Jiping - End Change
		{
...
void UAbilitySystemComponent::InternalServerTryActivateAbility(FGameplayAbilitySpecHandle Handle, bool InputPressed, const FPredictionKey& PredictionKey, const FGameplayEventData* TriggerEventData)
{
	...
	else
	{
		...
		//Han Jiping - Begin Change - To add the ClientActivateAbilityFailed workflow
		PostClientActivateAbilityFailed(Handle, PredictionKey.Current);
		//Han Jiping - End Change
