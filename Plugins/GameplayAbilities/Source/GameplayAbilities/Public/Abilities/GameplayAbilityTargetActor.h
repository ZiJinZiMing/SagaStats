...
public:
	...
	UE_API virtual void StartTargeting(UGameplayAbility* Ability);

	UE_API virtual bool IsConfirmTargetingAllowed();

	/** Requesting targeting data, but not necessarily stopping/destroying the task. Useful for external target data requests. */
	UE_API virtual void ConfirmTargetingAndContinue();

	/** Outside code is saying 'stop and just give me what you have.' Returns true if the ability accepts this and can be forgotten. */
	UFUNCTION()
	UE_API virtual void ConfirmTargeting();

	/** Outside code is saying 'stop everything and just forget about it' */
	UFUNCTION()
	UE_API virtual void CancelTargeting();

	UE_API virtual void BindToConfirmCancelInputs();

	UE_API virtual bool ShouldProduceTargetData() const;
	...
	UE_API virtual bool OnReplicatedTargetDataReceived(FGameplayAbilityTargetDataHandle& Data) const;
