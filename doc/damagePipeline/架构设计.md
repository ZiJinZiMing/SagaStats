...
**关键设计**：
- **Produces 通过 CDO 反查**，不存冗余字段——`UDamageRule::GetProducesEffectType()` 内部调 `OperationClass->CDO->GetEffectType()`；Operation 子类在构造函数里赋 `EffectType` 字段声明产出（和蓝图 Class Defaults 路径对齐）
...
class UDamageOperationBase : public UObject
{
    ...
    // 子类在构造函数里赋 EffectType 字段；基类默认实现读字段
    virtual UScriptStruct* GetEffectType() const { return EffectType; }

protected:
    UPROPERTY(EditAnywhere, meta=(EditCondition="IsClassDefaultContext", EditConditionHides))
...
**_Effect 示例**（推荐：构造函数赋 `EffectType` 字段，与蓝图子类在 Class Defaults 填字段的路径对齐）：

```cpp
UCLASS()
class UDamageCondition_IsLightning : public UDamageCondition_Effect
{
    UDamageCondition_IsLightning()
    {
        EffectType = FSekiroAttackContext::StaticStruct();
    }
    virtual bool Evaluate_Implementation(
        const UDamageContext* Context,
        const FInstancedStruct& InEffect) const override
    {
        const FSekiroAttackContext* Ctx = InEffect.GetPtr<FSekiroAttackContext>();
        return Ctx && Ctx->bIsLightning;
    }
};
```

> 基类默认实现 `UDamageCondition_Effect::GetEffectType()` 返回 `EffectType` 字段值；C++ 子类只需在构造函数里赋一次字段即可。高级用法可 `override GetEffectType()` 返回动态类型（罕见）。
...
**语法形式**（推荐：构造函数赋 `EffectType` 字段，对齐 `UDamageCondition_Effect` 的模式）：

```cpp
class UDamageOperation_Guard : public UDamageOperationBase
{
    UDamageOperation_Guard()
    {
        EffectType = FGuardEffect::StaticStruct();
    }
...
**Rule 层如何知道产出类型**（`UDamageRule::GetProducesEffectType()` 的实现）：

```
Rule.GetProducesEffectType()
  → OperationClass->GetDefaultObject<UDamageOperationBase>()->GetEffectType()
  → 基类默认读 CDO 的 EffectType 字段 → 返回构造函数赋的值
```

**为什么不在 Rule 上直接存 ProducesType 字段**：
- **避免冗余与不一致**：如果 Rule 也存字段，Operation 也存，两处必然漂移
- **CDO 反查保证一致性**：Rule 只保存 `OperationClass`，产出类型从 Operation CDO 的 `EffectType` 字段动态查询
...
- Operation 用它声明产出：在构造函数里赋 `EffectType` 字段（基类 `GetEffectType()` 读字段）
...
FGuardEffect::StaticStruct() 这个 UScriptStruct* 在三个位置必须一致：
   ...
   Rule.OperationClass CDO.GetEffectType()  // 读 CDO 的 EffectType 字段
            ↓
...
Step 1 — 构建 ProducerMap
  for each rule in DamageRules:
      ProducesType = rule.OperationClass CDO.GetEffectType()
...
for each sortedRule in SortedRules:
    ...
    // 2. 框架预创建 OutEffect
    ProducesType = sortedRule.OperationClass.CDO.GetEffectType()
...
- `OperationClass` 非空
- Operation CDO 的 `EffectType` 字段已声明（`GetEffectType()` 返回非空 `UScriptStruct*`）
...
**代码保障**：`UDamageContext::GetEffectByType(UScriptStruct*)` 在类型不存在时返回 invalid `FInstancedStruct`，`UDamageCondition_Effect::Evaluate_Implementation` 的 `InEffect` 参数此时为 invalid——Condition 作者用 `InEffect.GetPtr<T>() == nullptr` 检测。
...
`Plugins/SagaStats/Source/SagaStats/Public/DamagePipeline/Sekiro/` 下是只狼 MVP 的**最小验证集**——5 个 Rule 覆盖核心机制链路，证明本 DSL 能表达典型动作游戏伤害管线。

| 文件 | 机制 |
|---|---|
| SekiroAttackContext.h | 攻击事件原始描述（`FSekiroAttackContext` USTRUCT，Game 侧通过 `UDamagePipelineResults::WriteEffect` 预填入 DC；无 Producer） |
...
每条 DR 都是**独立 DamageRule + Operation + Effect + 专用 Condition 子类**的四件套。测试 Actor：`ADamagePipelineTestActor`（按 1-3 键触发 3 个典型场景：NormalHit / Guard / JustGuard）。

> 早期版本还有 DR_Death / DR_Poison / DR_LightningInAir / DR_LightningOnGround / DR_SuperArmor / DR_Toughness / DR_AttackerBound 等——这些是**设计叙事里的例子**（比如 §11.3 / §11.5 用来讲"涌现自治"或"涌现识别"的），不再作为 C++ 测试单元存在。未来可通过蓝图 / 新的 C++ Rule 扩展。
