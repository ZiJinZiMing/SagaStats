...
**调研日期: 2026-03-25（术语于 2026-04-13 更新至 v4.6）
**信息源**: 项目全量调研资料（设计观点、模型规范 v4.7（代码 v4.6）、实现层设计文档、案例分析、行业理论）
...
| [GraphEditor实现](./GraphEditor实现.md) | Graph 编辑器原理与代码清单（阶梯 Y + 通道 X 双重唯一性布局） | 程序 |
...
| 设计思想 | 架构策略 | 代码承载 |
|---------|---------|---------|
| 机制优先 | DamageRule（机制处理体）自洽封装 | 每个机制是独立的积木块 |
| 数值兜底 | DamageRule 内部完成数值结算 | 数学地基正确但不外溢 |
...
解决方案的核心是三个概念：**DamageRule + DC + Condition**

```mermaid
graph LR
    DC["DC<br/>共享上下文<br/>（共享工作台）"]
    DPU1["DR_Guard<br/>格挡处理体"]
    DPU2["DR_Lightning<br/>雷击处理体"]
    DPU3["DR_Death<br/>死亡处理体"]
...
| 概念 | 是什么 | 承载什么设计思想 |
|------|--------|----------------|
| **DamageRule** | 机制处理体——自洽的积木块 | 机制优先 + 数值兜底 |
...
动作游戏的伤害管线由四条设计思想驱动——**机制优先、数值兜底、反馈清晰可感、涌现自治**。管线分为机制阶段和表现选取阶段。机制阶段用声明式编排工具实现——DamageRule 是自洽的积木块（声明 produces），Condition 是装配门控（隐含 consumes，缺失字段视为 false），DC 是共享工作台。系统自动拓扑排序，支持动态管线。表现选取阶段按 Channel 分组、按 Priority 择优。开发时配规则，运行时配组合，两者解耦。
