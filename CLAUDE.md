...
MySagaStats 是一个 Unreal Engine 5.7 项目，核心是 **SagaStats 插件**中的 **DamageFramework**（原 DPU Framework）——用于动作游戏模块化伤害管线的声明式编排框架。代码当前版本为 **v4.6**（实现层），模型规范文档版本为 **v4.7**。
...
## 核心架构：DamageFramework（v4.6 代码 / v4.7 模型）

代码位于 `Plugins/SagaStats/Source/SagaStats/` 的 `Public/DamageFramework/` 和 `Private/DamageFramework/`。
...
| 概念 | 实现类 | 职责 |
|---|---|---|
| **DamageRule** | `UDamageRule` (DataAsset) | 规则定义：RuleName + Condition 树 + OperationClass + BasePriority。`GetProducesEffectType()` 从 OperationClass CDO 反查 |
...
6. **ConsumedEffectType HideInDetailPanel** — 子类的类级属性，实例不可修改
...
└── Private/DamageFramework/     — 对应 .cpp 实现
```
...
```cpp
// Public/DamageFramework/DamageContextLibrary.h
UCLASS()
...
SagaStatsEditor/
├── Public/Graph/
│   ├── DamagePipelineGraph.h       — UEdGraph 子类，RebuildGraph 自动布局
│   ├── DamagePipelineGraphNode.h   — UEdGraphNode，持有 UDamageRule 引用
│   └── DamagePipelineGraphSchema.h — 只读 Schema（禁止手动连线）
└── Private/
    ├── Graph/
    │   ├── SDamagePipelineGraphNode.h/cpp — Slate 节点外观 + NodeFactory
    │   └── ... 上述 .cpp
    ├── DamagePipelineAssetEditor.h/cpp    — FAssetEditorToolkit（双面板+Build按钮）
    └── DamagePipelineAssetTypeActions.h/cpp — 双击资产打开自定义编辑器
```

**自动布局**：按拓扑层级从左到右排列（`X = Layer * 400, Y = IndexInLayer * 200`），与 Mermaid `graph LR` 输出一致。
