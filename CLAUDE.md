...
MySagaStats 是一个 Unreal Engine 5.7 项目，核心是 **SagaStats 插件**中的 **DamagePipeline**（原 DPU Framework / DamageFramework）——用于动作游戏模块化伤害管线的声明式编排框架。代码与模型规范当前版本均为 **v4.7**。
...
- `doc/damagePipeline/_archive/已解决决策_v4.3-v4.7.md` — 13 条历史决策归档（含 v4.6→v4.7 演进期的 7 项关键架构决策）
- `doc/damagePipeline/待讨论问题集.md` — 当前活跃讨论（Part A/B/C）与 v4.7 实施状态附录
- `doc/damagePipeline/README.md` — 全员入口，含 L3=领域 DSL 客观定义论证
...
| **Condition 树** | `UDamagePredicate`(容器) + `UDamageCondition`(抽象基类) + `UDamageCondition_Effect` / `UDamageCondition_Context`(具体子类) | Predicate 负责 AND/OR/Single + bReverse；Condition 分两类：_Effect 绑定 EffectType 贡献 R5 依赖，_Context 不贡献依赖只看 DC 本身（v4.7） |
| **DamagePipelineResults** | `UBlueprintFunctionLibrary` | Game 侧读写 DC 的**唯一外部 API 入口**（`WriteEffect`/`ReadEffect`/`HasEffect`/`GetAllEffects`）；v4.7 引入以配合 DC 访问控制收紧 |
...
这些决策是 v4.6→v4.7 演进期的实现层选择，现已成为 v4.7 官方形态。历史归档详见 `doc/damagePipeline/_archive/已解决决策_v4.3-v4.7.md`：
...
### DC 访问分层（v4.7+）与蓝图 API

**设计目标**：R5 契约（Condition/Operation 不得越界读其他 Effect）从"文档约束"升级为"编译器强制"。

`UDamageContext` 的 Effect 读写 API 全部 `protected`，friend 列表：
- `UDamagePipeline`（Build/Execute 内部）
- `UDamagePipelineResults`（Game 侧读写包装——**唯一外部入口**）
- `UDamageCondition_Effect`（按 EffectType 预取）
- `UDamageOperationBase`（通过基类 `ReadEffect<T>` 模板给 Operation 子类使用；R5 侧未强制，留下轮）

**Game 侧 C++**：`UDamagePipelineResults::ReadEffect<T>(Ctx)` / `WriteEffect<T>(Ctx, Val)` / `HasEffect<T>(Ctx)` / `GetAllEffects(Ctx)`

**Game 侧蓝图**：节点名 "Read Pipeline Result" / "Write Pipeline Input"（CustomThunk 通配结构体）

**DSL 内部 C++**：
- Condition_Effect 子类：`Evaluate(Context, InEffect)` —— 框架已预取 InEffect，无需再调 Context
- Condition_Context 子类：`Evaluate(Context)` —— **设计上禁止读 Effect**（签名里没有 InEffect，Context 的 GetEffect 被编译器拒绝）
- Operation 子类：`ReadEffect<T>(Context)` —— 基类提供，本轮不做 R5 校验

**重要约束**：
- ❌ 不能直接在 UDamageContext 成员上定义 CustomThunk + CustomStructureParam（会因缺少 K2Node 导致 Stack 解析错乱崩溃）
- ❌ Condition/Operation 子类代码里 `Context->GetEffect<T>()` 会编译错误（protected）——这是**预期**的类型契约强制
- ✅ Game 扩展字段仍可通过 `Cast<UDamageContextSubclass>(Context)->GameField` 访问
...
任何 UClass/UStruct/UPROPERTY 的重命名操作**必须**在 `Plugins/SagaStats/Config/DefaultSagaStats.ini` 的 `[CoreRedirects]` 节中添加对应条目，否则蓝图资产引用会丢失。当前已包含历史版本到 v4.7 的完整重命名映射（DPU→DamageRule、Fact→Effect、Logic→Operation 等约60条）。
...
- **双端协作** — 代码与模型规范当前均维护在 v4.7。实现层的历史决策已归档至 `doc/damagePipeline/_archive/`。未来若再出现代码与模型规范并行维护的情况，需要输出变更记录同步给维护模型规范的一方
